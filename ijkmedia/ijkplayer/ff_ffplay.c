...
#pragma  mark - 录制插入
        ...
        if (ffp->is_record && !ffp->is->paused) { // 可以录制时，写入文件
...
int ffp_start_recording_l(FFPlayer *ffp,const char *file_name)
{
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        if (mediaType == AVMEDIA_TYPE_AUDIO) {
            if (av_stream->codec->codec_id == AV_CODEC_ID_PCM_ALAW) {
                isPCM = 1;
            }
            ffp->audio_index = i;
    ...
    if (isPCM) {
        fmt = av_guess_format("mov", NULL, "video/mov");
        avformat_alloc_output_context2(&ffp->m_ofmt_ctx, fmt, NULL, NULL);
    }else {
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        if (i == ffp->audio_index || i == ffp->video_index) {
            // 对照输入流创建输出流通道
...
int ffp_get_current_frame(FFPlayer *ffp, const char *file_name) {
    ...
    if (!ffp->is_screenshot){
            ...
            ffp->screen_file_name = (char*)malloc(sizeof(char)*strlen(file_name)+1);
            strcpy(ffp->screen_file_name, file_name);
            return 1;
        }
    return 0;
}
